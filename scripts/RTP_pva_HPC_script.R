#17/2/2025 Phascogale PVA test on HPC
...
parDefault.list$maleAdultSR <- 0.8
...
parRange.list$maleAdultSR <- c(0.2,1)
...
## tree.complexities to test with the BRT emulators (tree.complexities denotes order of interaction terms)
resp <- 'Extant'
tree.complexities <- c(1,2,5)

## subsample sizes to test during the emulation step
(subsamples <- c(250, 500, 1000, 5000, seq(10000,nSims,by=10000))) #for 100,000 sims
#(subsamples <- c(250, 500, 1000, seq(2000, nSims, by=2000))) #for 10,000 sims

## emulate the sensitivity analysis output for the required response variable
brt.models <- foreach (i=length(subsamples):1, .verbose=T, .packages=c('dismo', 'gbm')) %dopar% {
...
emulation.results <- emulation.summary.func(subsamples=subsamples, tree.complexities=tree.complexities, brt.models = brt.models)


#save emulation results as an r.data file
save(emulation.results, file = 'emulation.results.Rdata') #use on HPC to export data
